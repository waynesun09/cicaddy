...
- Gemini via Vertex AI (`gemini-vertex` provider) — uses Google Cloud ADC with auto-fallback from API key auth (v0.11.0+)
...
- Run `uv run pytest tests/unit/ -q` before committing (must pass 1200+ tests)
