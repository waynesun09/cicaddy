"""Cron-specific AI Agent for scheduled analysis tasks."""
...
class CronAIAgent(BaseAIAgent):
    """AI Agent for scheduled cron jobs without MR dependency."""

    def __init__(self, settings: Optional[Settings] = None):
        super().__init__(settings)

        # Cron-specific configuration
        self.task_type = os.getenv("CRON_TASK_TYPE", "scheduled_analysis")
        # CRON_SCOPE is only relevant for code/project analysis tasks, not external MCP monitoring
        scope_env = os.getenv("CRON_SCOPE")
        ...
        self.report_format = os.getenv("CRON_REPORT_FORMAT", "detailed")

    async def initialize(self):
        """Initialize all components for cron mode."""
        logger.info(
            "Initializing AI Agent for cron job",
        ...
        if (
            ...
            logger.info(
                f"Initializing Cron Slack notifier with {len(slack_webhook_urls)} webhook(s)"
            )
        ...
        logger.info("Cron AI Agent initialized successfully")
    ...
    async def run_scheduled_analysis(self) -> Dict[str, Any]:
        ...
        # Return in expected format with cron-specific metadata
        return {
            "task_type": self.task_type,
            "scope": self.scope,
        ...
        if self.gitlab_analyzer:
            try:
                ...
                if "AuthenticationError" in error_type or "Unauthorized" in str(e):
                    ...
                    # Catch-all for unexpected errors with full traceback for debugging cron jobs
                    logger.error(
    ...
    def _build_custom_prompt(self, context: Dict[str, Any]) -> str:
        ...
        return self._build_cron_prompt(custom_prompt, context)
...
"""
        return self._build_cron_prompt(security_prompt, context)
...
"""
        return self._build_cron_prompt(quality_prompt, context)
...
"""
        return self._build_cron_prompt(dependency_prompt, context)
...
"""
        return self._build_cron_prompt(general_prompt, context)

    def get_session_id(self) -> str:
        """
        Get unique session ID for this cron analysis session.
        ...
        if job_id:
            return f"cron_{self.task_type}_{job_id}"
        # Fallback for local development (still non-deterministic)
        return f"cron_{self.task_type}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    ...
    async def generate_report(
        ...
        """Generate formatted report with cron-specific details."""
    ...
    def _build_cron_prompt(self, custom_prompt: str, context: Dict[str, Any]) -> str:
